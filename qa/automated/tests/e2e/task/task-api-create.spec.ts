...
function uniqueEmail(): string {
  return `taskcreate_${Date.now()}_${Math.floor(Math.random() * 100000)}@example.com`;
}
...
async function registerAndLogin(
  ...
  const fullName = overrides?.fullName ?? "Task Create Test User";
  ...
  return {
    id: registerBody.id ?? loginBody.owner.id,
    email,
    fullName,
    password,
...
/** Returns a future date string 30 days from now in ISO format */
