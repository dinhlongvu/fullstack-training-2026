...
## TC-TASK-COMMENTS-008: Empty state (task has no comments)

| Field | Content |
|-------|----------|
| **Test Case ID** | TC-TASK-COMMENTS-008 |
| **Type** | Positive |
| **Technique** | Boundary / Empty state |
| **Precondition** | User is logged in; task exists but has 0 comments |
| **Test Data** | Authorization: `Bearer <token>`, taskId: valid |
| **Test Steps** | 1. Send GET request to `/api/tasks/{id}/comments` <br> 2. Check response status code and data |
| **Expected Result** | 1. Status 200 OK <br> 2. Returns an empty array `[]` |
| **Actual Result** | 1. Response returned status 200 OK <br> 2. Data is `[]` |
| **Status** | ✅Pass |
| **Bug link** | — |

---

## TC-TASK-COMMENTS-009: Check comment sorting order

| Field | Content |
|-------|----------|
| **Test Case ID** | TC-TASK-COMMENTS-009 |
| **Type** | Positive |
| **Technique** | Data Validation |
| **Precondition** | User is logged in; task exists and has multiple comments |
| **Test Data** | Authorization: `Bearer <token>`, taskId: valid |
| **Test Steps** | 1. Send GET request to `/api/tasks/{id}/comments` <br> 2. Check the `createdAt` property of the returned comments |
| **Expected Result** | 1. Status 200 OK <br> 2. Comments should be sorted by `createdAt` in ascending order (oldest first) |
| **Actual Result** | 1. Response returned status 200 OK <br> 2. Comments are sorted correctly |
| **Status** | ✅Pass |
| **Bug link** | — |
